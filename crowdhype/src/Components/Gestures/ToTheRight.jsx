...
const ToTheRight = ({
  ...
  const fontSize = 0.24;
  const numPointsActualPath1 = 35;
  const numPointsActualPath2 = 15;
  ...
  //paths
  const pathRadius1 = 2.4;
  const pathArc1 = Math.PI / 2;
  const pathPosition1 = new THREE.Vector3(1.8, 1, zPosition);
  const pathRotation1 = new THREE.Euler(0, Math.PI, 0);
  const pathRadius2 = 1;
  const pathArc2 = Math.PI / 2;
  const pathPosition2 = new THREE.Vector3(1.8, 1, zPosition);
  const pathRotation2 = new THREE.Euler(0, Math.PI, 0);

  const conePosition1 = new THREE.Vector3(2, 3.4, zPosition);
  const coneRotation1 = new THREE.Euler(0, 0, Math.PI * 1.5);
  const conePosition2 = new THREE.Vector3(2, 2, zPosition);
  const coneRotation2 = new THREE.Euler(0, 0, Math.PI * 1.5);

  const endPosition1 = new THREE.Vector3(2.3, 3.4, zPosition);
  const endPosition2 = new THREE.Vector3(2.3, 2, zPosition);

  const textPosition = new THREE.Vector3(-1.4, 2, zPosition);
  const textRotation = new THREE.Euler(0, 0, 0.8);

  useEffect(() => {
    setActualPath1(getPath(
      pathRadius1, pathArc1, numPointsActualPath1, pathPosition1, pathRotation1
    ));

    setActualPath2(getPath(
      pathRadius2, pathArc2, numPointsActualPath2, pathPosition2, pathRotation2
    ));
  }, []);
  ...
  useFrame(() => {
    ...
    setScoreOpacity(
      showScore ? Math.min(scoreOpacity + animationSpeed, 1) : Math.max(scoreOpacity - animationSpeed, 0)
    );
    ...
    <>
      ...
      >
        <mesh position={pathPosition1} rotation={pathRotation1}>
          <torusGeometry args={[pathRadius1, 0.13, 30, 30, pathArc1]} />
          <meshBasicMaterial
      ...
      >
        <mesh position={pathPosition2} rotation={pathRotation2}>
          <torusGeometry args={[pathRadius2, 0.13, 30, 30, pathArc2]} />
          <meshBasicMaterial
